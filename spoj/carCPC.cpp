...
#define OLD(X) (!isalpha((X)[3]) && !isalpha((X)[4]))
#define BADCHAR(ch) ((ch)=='A'||(ch)=='C'||(ch)=='P'||(ch)=='I'||(ch)=='M')
#define BAD(x) (isalpha(x[3]) && !isalpha(x[4])) || (!isalpha(x[3]) && isalpha(x[4]))
#define DEBUG 0
using namespace std;
...
int getDiff(string str1, string str2)
{
	...
	if( old1 && ! old2) {
		val1 = getVal1(str1);
		val2 = getVal2(str2);
		int oldBase = getVal1("ZZZ9999");
		return (val2 + oldBase + 1 - val1);
	}
	if (old1) {
		val1 = getVal1(str1);
		val2 = getVal1(str2);
